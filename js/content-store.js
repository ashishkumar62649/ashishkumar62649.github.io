...
let siteContent = structuredClone(defaultContent);
let phaseWords = [];
let stackRows = [];
...
const setText = (selector, value) => {
  ...
  if (element) element.textContent = richText(value);
};

const setHtml = (selector, value) => {
  const element = $(selector);
  if (element) element.innerHTML = renderLinkedText(value);
};
...
const mergeContent = (content = {}) => ({
  ...
  contact: {
    ...
    socialLinks: normalizeArray(content.contact?.socialLinks)
  },
...
const loadContent = async () => {
  try {
    siteContent = mergeContent(await fetchJson("./content/site.json"));
  } catch {
    try {
      siteContent = mergeContent(readEmbeddedContent());
    } catch {
      siteContent = structuredClone(defaultContent);
    }
  }
};
